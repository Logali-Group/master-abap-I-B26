CLASS ycl_sql_lab_b26 DEFINITION PUBLIC FINAL CREATE PUBLIC.

  PUBLIC SECTION.
    INTERFACES if_oo_adt_classrun.

  PRIVATE SECTION.
    TYPES tt_emp TYPE STANDARD TABLE OF ysql_emp_b26 WITH EMPTY KEY.
...
CLASS YCL_SQL_LAB_B26 IMPLEMENTATION.
  ...
  METHOD reset.
    DELETE FROM ysql_emp_b26 WHERE emp_id < 1000.
    COMMIT WORK.
  ...
  METHOD b2_insert.
    ...
    DATA(ls_ana) = VALUE ysql_emp_b26( emp_id     = 1
                                       first_name = 'Ana'
                                       last_name  = 'Gómez'
                                       emp_role   = 'Developer'
                                       city       = 'Madrid'
    ...
    INSERT ysql_emp_b26 FROM @ls_ana.
    ...
    INSERT ysql_emp_b26 FROM TABLE @lt_emp.
    ...
    TRY.
        INSERT ysql_emp_b26 FROM TABLE @lt_dup.
    ...
    INSERT ysql_emp_b26 FROM TABLE @lt_dup ACCEPTING DUPLICATE KEYS.
    traza( 'El mismo grupo, ahora tolerante' ).
    COMMIT WORK.

    SELECT COUNT(*) FROM ysql_emp_b26 WHERE emp_id < 1000 INTO @DATA(lv_total).
  ...
  "───── 3.1 · Actualizar UN registro (fila completa) ─────
    ...
    SELECT SINGLE FROM ysql_emp_b26
      FIELDS *
      WHERE emp_id = 1
      INTO @DATA(ls_emp).
    ...
    UPDATE ysql_emp_b26 FROM @ls_emp.
    traza( 'UPDATE fila completa' ).

    " clave que no existe: no crea nada
    UPDATE ysql_emp_b26 FROM @( VALUE ysql_emp_b26( emp_id = 999 last_name = 'Fantasma' ) ).
    ...
    UPDATE ysql_emp_b26 FROM TABLE @lt_dev.
    ...
    SELECT FROM ysql_emp_b26 FIELDS emp_id, last_name, emp_role, city, salary
      WHERE emp_id < 1000 ORDER BY emp_id INTO TABLE @DATA(lt_final).
    out->write( lt_final ).
  ...
  METHOD b4_modify.
    ...
    DATA(ls_new) = VALUE ysql_emp_b26( emp_id     = 10
                                       first_name = 'Elena'
                                       last_name  = 'Navarro'
                                       emp_role   = 'Developer'
                                       city       = 'Zaragoza'
    ...
    MODIFY ysql_emp_b26 FROM @ls_new.
    ...
    MODIFY ysql_emp_b26 FROM TABLE @lt_mix.
  ...
  METHOD b5_delete.
    ...
    DELETE FROM ysql_emp_b26 WHERE emp_id = 12.
    traza( 'DELETE … WHERE clave' ).

    " variante: la clave viene en un área de trabajo
    DELETE ysql_emp_b26 FROM @( VALUE ysql_emp_b26( emp_id = 11 ) ).
    traza( 'DELETE … FROM @wa' ).

    " clave que no existe
    DELETE FROM ysql_emp_b26 WHERE emp_id = 998.
    traza( 'DELETE de una clave inexistente' ).

    "───── 5.2 · Eliminar MÚLTIPLES registros ─────
    titulo( '5.2 · DELETE masivo desde tabla interna' ).

    DATA(lt_bajas) = VALUE tt_emp( ( emp_id = 6 ) ( emp_id = 7 ) ).
    DELETE ysql_emp_b26 FROM TABLE @lt_bajas.
    traza( 'DELETE … FROM TABLE (solo se usa la clave)' ).

    "───── 5.3 · Eliminar con FILTROS ─────
    titulo( '5.3 · DELETE con condiciones' ).

    DELETE FROM ysql_emp_b26 WHERE active = '' AND emp_id < 1000.
    traza( 'Baja de los inactivos' ).

    DELETE FROM ysql_emp_b26 WHERE hire_date < '20200101' AND emp_id < 1000.
    traza( 'Baja de los anteriores a 2020' ).

    "  ☠️  DELETE FROM ysql_emp_b26.   ← SIN WHERE: borra TODA la tabla (del mandante actual)

    SELECT COUNT(*) FROM ysql_emp_b26 WHERE emp_id < 1000 INTO @DATA(lv_quedan).
  ...
  METHOD b6_luw.
    ...
    SELECT SINGLE FROM ysql_emp_b26 FIELDS city WHERE emp_id = 1 INTO @DATA(lv_city).
    out->write( |   Ciudad antes:            { lv_city }| ).

    UPDATE ysql_emp_b26 SET city = 'ERROR-DE-CARGA' WHERE emp_id = 1.

    SELECT SINGLE FROM ysql_emp_b26 FIELDS city WHERE emp_id = 1 INTO @lv_city.
    ...
    UPDATE ysql_emp_b26 SET city = 'Valencia' WHERE emp_id = 1.
    COMMIT WORK.
  ...
  METHOD b7_select.
    ...
    SELECT FROM ysql_emp_b26 FIELDS emp_id, last_name, city
      WHERE city = 'Madrid' AND emp_id < 1000
    ...
    TYPES: BEGIN OF ty_ficha,
             city      TYPE ysql_emp_b26-city,
             last_name TYPE ysql_emp_b26-last_name,
    ...
    SELECT FROM ysql_emp_b26 FIELDS salary WHERE emp_id < 1000 INTO @DATA(lv_sal).
      lv_suma = lv_suma + lv_sal.
    ENDSELECT.
    out->write( |   Suma calculada fila a fila en ABAP:      { lv_suma }| ).

    SELECT FROM ysql_emp_b26 FIELDS SUM( salary ) WHERE emp_id < 1000 INTO @DATA(lv_suma_sql).
